...
/**
 * Created by yzd on 2018/6/20 0020.
 */
...
public class BluetoothService {
    ...
    public void write(byte[] out) {
        ...
        int count = r.getIO().Write(out, 0, out.length);
        if (count > -1) {
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (mState == STATE_CONNECTED) {
                ...
                bytes = io.Read(buffer, 0, buffer.length, 100);
                if (bytes != -1) {
                    if (bytes > 0)
                        // Send the obtained bytes to the UI Activity
                        mHandler.obtainMessage(Constants.MESSAGE_READ, bytes, -1, Arrays.copyOf(buffer, bytes)).sendToTarget();
                } else {
