 ...
 *     time  : 2017/05/24
 *     desc  : utils about memory cache(内存缓存工具)
 *     menu
 *          getInstance           : 1.获取缓存实例
            Instance.put          : 2.缓存中写入数据
            Instance.get          : 3.缓存中读取字节数组
            Instance.getCacheCount: 4.获取缓存个数
            Instance.remove       : 5.根据键值移除缓存
            Instance.clear        : 6.清除所有缓存
 * </pre>

 * Created by yzd on 2018/7/5 0005.
 */
