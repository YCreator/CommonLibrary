package com.frame.core.bluetooth.core;
...
import com.frame.core.bluetooth.callback.IRssiCallback;
...
import static com.frame.core.bluetooth.common.BleConstant.MSG_CONNECT_RETRY;
import static com.frame.core.bluetooth.common.BleConstant.MSG_CONNECT_TIMEOUT;
import static com.frame.core.bluetooth.common.BleConstant.MSG_READ_DATA_RETRY;
import static com.frame.core.bluetooth.common.BleConstant.MSG_READ_DATA_TIMEOUT;
import static com.frame.core.bluetooth.common.BleConstant.MSG_RECEIVE_DATA_RETRY;
import static com.frame.core.bluetooth.common.BleConstant.MSG_RECEIVE_DATA_TIMEOUT;
import static com.frame.core.bluetooth.common.BleConstant.MSG_WRITE_DATA_RETRY;
import static com.frame.core.bluetooth.common.BleConstant.MSG_WRITE_DATA_TIMEOUT;
...
public class DeviceMirror {
    ...
    private BluetoothGattCallback coreGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(final BluetoothGatt gatt, final int status) {
            ...
            if (status == 0) {
                ...
                if (connectCallback != null) {
                    ...
                    Bluetooth.getInstance().getDeviceMirrorPool().addDeviceMirror(deviceMirror);
        ...
        public void onCharacteristicRead(BluetoothGatt gatt, final BluetoothGattCharacteristic characteristic, final int status) {
            LogUtils.i("onCharacteristicRead  status: " + status + ", data:" + HexUtil.encodeHexStr(characteristic.getValue()) +
                    "  ,thread: " + Thread.currentThread());
            if (status == BluetoothGatt.GATT_SUCCESS) {
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, final BluetoothGattCharacteristic characteristic, final int status) {
            LogUtils.i("onCharacteristicWrite  status: " + status + ", data:" + HexUtil.encodeHexStr(characteristic.getValue()) +
                    "  ,thread: " + Thread.currentThread());
            if (status == BluetoothGatt.GATT_SUCCESS) {
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, final BluetoothGattCharacteristic characteristic) {
            LogUtils.i("onCharacteristicChanged data:" + HexUtil.encodeHexStr(characteristic.getValue()) +
                    "  ,thread: " + Thread.currentThread());
        ...
        public void onDescriptorRead(BluetoothGatt gatt, final BluetoothGattDescriptor descriptor, final int status) {
            LogUtils.i("onDescriptorRead  status: " + status + ", data:" + HexUtil.encodeHexStr(descriptor.getValue()) +
                    "  ,thread: " + Thread.currentThread());
            if (status == BluetoothGatt.GATT_SUCCESS) {
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, final BluetoothGattDescriptor descriptor, final int status) {
            LogUtils.i("onDescriptorWrite  status: " + status + ", data:" + HexUtil.encodeHexStr(descriptor.getValue()) +
                    "  ,thread: " + Thread.currentThread());
            if (status == BluetoothGatt.GATT_SUCCESS) {
        ...
        public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
            LogUtils.i("onReadRemoteRssi  status: " + status + ", rssi:" + rssi +
                    "  ,thread: " + Thread.currentThread());
            if (status == BluetoothGatt.GATT_SUCCESS) {
    ...
    public synchronized void connect(IConnectCallback connectCallback) {
        if (connectState == ConnectState.CONNECT_SUCCESS || connectState == ConnectState.CONNECT_PROCESS
            ...
            LogUtils.e("this connect state is connecting, connectSuccess or current retry count less than config connect retry count.");
            return;
        }
        if (handler != null) {
            handler.removeCallbacksAndMessages(null);
    ...
    public void writeData(byte[] data) {
        if (data == null || data.length > 20) {
            LogUtils.e("this data is null or length beyond 20 byte.");
            return;
        }
    ...
    private boolean checkBluetoothGattInfo(HashMap<String, BluetoothGattChannel> bluetoothGattInfoHashMap) {
        if (bluetoothGattInfoHashMap == null || bluetoothGattInfoHashMap.size() == 0) {
            LogUtils.e("this bluetoothGattInfo map is not value.");
            return false;
        }
        return true;
    }
    ...
    private synchronized void connect() {
        ...
        if (bluetoothLeDevice != null && bluetoothLeDevice.getDevice() != null) {
            bluetoothLeDevice.getDevice().connectGatt(Bluetooth.getInstance().getContext(), false, coreGattCallback);
        }
    }
    ...
    private void connectFailure(BleException bleException) {
        if (connectRetryCount < BleConfig.getInstance().getConnectRetryCount()) {
            ...
            LogUtils.i("connectFailure connectRetryCount is " + connectRetryCount);
        } else {
            ...
            LogUtils.i("connectFailure " + bleException);
        }
    }
    ...
    private void enableFailure(BleException bleException, boolean isRemoveCall) {
        if (receiveDataRetryCount < BleConfig.getInstance().getOperateRetryCount()) {
            ...
            LogUtils.i("enableFailure receiveDataRetryCount is " + receiveDataRetryCount);
        } else {
            handleFailureData(enableInfoMap, bleException, isRemoveCall);
            LogUtils.i("enableFailure " + bleException);
        }
    }
    ...
    private void readFailure(BleException bleException, boolean isRemoveCall) {
        if (readDataRetryCount < BleConfig.getInstance().getOperateRetryCount()) {
            ...
            LogUtils.i("readFailure readDataRetryCount is " + readDataRetryCount);
        } else {
            handleFailureData(readInfoMap, bleException, isRemoveCall);
            LogUtils.i("readFailure " + bleException);
        }
    }
    ...
    private void writeFailure(BleException bleException, boolean isRemoveCall) {
        if (writeDataRetryCount < BleConfig.getInstance().getOperateRetryCount()) {
            ...
            LogUtils.i("writeFailure writeDataRetryCount is " + writeDataRetryCount);
        } else {
            handleFailureData(writeInfoMap, bleException, isRemoveCall);
            LogUtils.i("writeFailure " + bleException);
        }
    }
