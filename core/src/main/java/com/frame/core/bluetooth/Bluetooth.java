...
public class Bluetooth {
    ...
    private static Bluetooth instance;//入口操作管理
    ...
    public void init() {
        if (BaseApplication.get_context() != null) {
            bluetoothManager = (BluetoothManager) BaseApplication.get_context().getSystemService(Context.BLUETOOTH_SERVICE);
        ...
        startScan(new SingleFilterScanCallback(new IScanCallback() {
            ...
            public void onScanFinish(final BluetoothLeDeviceStore bluetoothLeDeviceStore) {
                if (bluetoothLeDeviceStore.getDeviceList().size() > 0) {
                    new Handler(Looper.getMainLooper()).post(() -> connect(bluetoothLeDeviceStore.getDeviceList().get(0), connectCallback));
                } else {
